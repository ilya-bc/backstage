...
#### 1. Create a colaltor module package

This module will add a FAQ collator to the Backstage backend index registry once it gets installed.
We recommend that you create it in a separate package, let's say, something like `plugins/search-backend-module-faq-snippets-collator` and you can use the `backstage-cli` new command to help you with that:

1. Access you Backstage project root directory and run `npx @backstage/cli new`;
2. When asked about what do you want to create, please select `backend-module` and press enter;
3. Enter `search` as the plugin id and `faq-collator` as module id;
4. A folder `plugins/search-backend-module-faq-snippets-collator` should have been created in your project.
...
cd plugins/search-backend-module-faq-snippets-collator
...
#### 3. Define the collator document type
...
#### 4. Use Backstage App configuration
...
<!---
TODO: write about the optional schedule config
-->
...
export type FaqSnippetsCollatorFactoryOptions = {
  logger: Logger;
};
...
export class FaqSnippetsCollatorFactory implements DocumentCollatorFactory {
  ...
  private constructor(options: FaqSnippetsCollatorFactoryOptions) {
  ...
  static fromConfig(
    config: Config,
    options: FaqSnippetsCollatorFactoryOptions,
  ) {
...
Replace the `module.ts` file with the content below:
...
export const searchFaqCollatorModule = createBackendModule({
  pluginId: 'search',
  moduleId: 'faq-collator',
  register(env) {
    env.registerInit({
      deps: {
      ...
      async init({ config, logger, scheduler, indexRegistry }) {
        ...
        indexRegistry.addCollator({
          ...
          factory: FaqCollatorFactory.fromConfig(config, { logger }),
        });
      },
    });
  },
...
And export the module as default in the `index.ts` file:

```ts title='plugins/search-backend-module-faq-snippets-collator/src/index.ts'
export { export const searchFaqCollatorModule as default } from './module';
```

#### 7. Install the collator module

Add the newly created module to your backend package dependencies:

```sh
# navigate to the backend directory
# (for this tutorial our backend lives in the backstage repo, if your backend lives in a separate repo you need to clone that first)
cd ../package/backend

# Install the module
yarn add backstage-plugin-search-backend-module-faq-snippets-collator
```

And install the module in your Backstage backend instance:
...
// Installing the newrly created faq collator module.
backend.add(import('backstage-plugin-search-backend-module-faq-snippets-collator');
//...
backend.start();
```

#### 8. Test your collator
...
#### 9. Make your plugins collator discoverable for others
